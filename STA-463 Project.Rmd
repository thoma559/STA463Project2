...
$$Y_i=\beta_0+\beta_1X_{1i}+\beta_2X_{2i} +\beta_3X_{3i} +\beta_4X_{1i}X_{2i}+\epsilon_i,\ i=1,...,397$$
...
The way to measure whether a point has too high of leverage, is to use the equation $3*\frac{p}{n}$. In our case we have 5 β parameters and 397 observations. Therefore leverage points of greater than .03778 should be removed to evaluate how much of difference these observations have on the output.
...
We can see that our leverage plot looks much better with the high leverage points being eliminated so now we will look to see if our assumptions are stronger without the influential points.
...
We can now evaluate our assumptions based on eliminating our most influential observations from the data. To see if our assumptions have gotten better we will reevaluate them. We should look to see if the correct model and constant variance assumptions are met. Looking at the standardized residual plots we don't see too many trends that represent fanning or tunneling that are concerning so this assumption looks to be met. The data has a lot of data so the clustering of the data in the center is not of concern. Therefore comparing the graphs from all the observations we do not see much of a difference in our assumption of constant variance and model form. 

The normality assumption looks a little better when comparing the histograms because without the influential points the histogram is much more symmetric. The QQ plot looks the same between the two models.

The last assumption should focus on the influential points which was the whole point of this model so we know they have been taken care of. Therefore getting rid of the most influential points does not seem to make our assumptions much stronger and there does not seem to be a reason to get rid of this most influential points. 
...
summary(fit)
```
We can see that the summary outputs are very similar and again affirming out intuition that we do not need get rid of the most influential points. So we will now continue with the fitted model of all our observations.

For our hypothesis question, we test the following hypotheses: 
Null Hypothesis: $\beta_4 = 0$
Alternate Hypothesis: $\beta_4 \neq 0$

According to the output, we reject the null hypothesis as the p-value for the t-test is approximately 0.001 (less than 0.05).We conclude that the interaction term between years of service and discipline is statistically significant. 
...
$$\hat{Y_i}=90700.5+485.6X_i + 9060.4X_{iii},\ i=1,...,16$$

And the fitted model to predict the salary of a professor in an applied field would be: 
$$\hat{Y_i}=(90700.5 + 392.9)+(485.6 + 392.9 )X_i + 9060.4X_{iii},\ i=1,...,16$$
$$\hat{Y_i}=91093.4+878.5X_i + 9060.4X_{iii},\ i=1,...,16$$
where $\hat{Y_i}$ is the estimated salary, $X_i$ is the years of service, and $X_{iii}$ is the sex variable. 
