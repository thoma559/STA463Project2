...
dataFrame <- salaries_data[abs(levs)<.05,]
...
From these plots, we can see that when we eliminated our most influential observation in the data we can see this did not make a difference in our assumptions. The normality assumption looks he same and like stated above we believe it looks pretty good. Our residual plots look nearly identical, and again as stated above we believe our constant variance assumption looks pretty good with only slight fanning. The model form looks appropriate like stated above. Therefore getting rid of the most influential point does not seem to make our assumptions stronger and there does not seem to be a reason to get rid of this most influential point.
...
summary(fit)
```
We can see that the summary outputs are very similar and again affirming out intuition that we do not need get rid of the most influential point. So we will now continue with the fitted model of all our observations.
